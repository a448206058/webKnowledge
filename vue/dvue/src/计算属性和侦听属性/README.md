## 序言：
自我学习的时候，总是感觉疲于阅读，缺乏深入性思考，所以决定从这篇文章开始对已有知识进行归纳和总结，希望能更多一点的自我思考。

自己的目标是结构性的梳理和阐述已有知识脉络，如果在这个过程中能帮助到你更深刻的了解相关知识，这将使我感到无比高兴（^_^）。

## 计算属性和监听属性
本文主要是介绍计算属性(**computed**)和监听属性(**watcher**)的原理以及它们是如何在vue内部运行的，示例和用法在vue的官网都有通俗的介绍，这里只会简单介绍相关概念，如果有不了解的同学可以先去了解下。
...
1. 适用场景：
    ...
    监听属性适用于观测某个值的变化去完成一段复杂的逻辑业务，允许我们执行异步操作，限制我们执行该操作的频率，并在我们得到最终结果前，设置中间状态。这些都是计算属性无法做到的。
