 ...
 * 这个值会被作为this参数传递给目标函数当bound函数被调用的时候。如果使用new运算符构造bound函数，则忽略该值。使用bind在setTimeout内创建函数（
 * 作为回调函数提供）时，作为thisArg传递的任何原始值都将转换为object.如果没有参数提供给bind，或者thisArg是null或者undefined则执行范围的
 * this将被视为新函数的this参数
 *
